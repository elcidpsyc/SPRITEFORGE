import { ACTION_META, ACTIONS, CELL, DIRECTIONS, HITBOX, PIVOT } from "../contract";
import type { Atlas } from "../types";

export function buildSpriteFramesJson(atlas: Atlas) {
  const animations: Array<{
    name: string;
    speed: number;
    loop: boolean;
    frames: Array<{ texture: string; x: number; y: number; w: number; h: number }>;
  }> = [];
  ...
  for (const action of ACTIONS) {
    ...
    DIRECTIONS.forEach((dir, dirIndex) => {
      ...
      if (meta.hitFrame !== undefined) hitFrames[name] = meta.hitFrame;
    });
  }
...
export const GODOT_IMPORTER = `@tool
...
export const GODOT_README = `# SPRITEFORGE → Godot 4.x
...
export const GODOT_TSCN_EXAMPLE = `; SPRITEFORGE example scene (generated by spriteforge_importer.gd)
